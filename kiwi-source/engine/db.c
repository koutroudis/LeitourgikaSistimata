...
{
	 pthread_mutex_lock(&mergeSafe);//kleidwnoume gia na elexoume an xreiazetai merge
	 test = 1;//thetoume thn global metavlhth iso me 1, wste na gnwrizei to programma ana pasa stigmh an ginetai merge
    ...
    test = 0;//thetoume thn global metavlhth iso me 0, wste na gnwrizei to programma oti to merge oloklhrwthhke 
    pthread_cond_broadcast(&mergeWait);//stelnoume shma se ola ta nhmata pou perimenoun
	 pthread_mutex_unlock(&mergeSafe);//xekleidwnoume otan teleiwsei to merge (an ginei).
...
int db_get(DB* self, Variant* key, Variant* value)
{
	 ...
	 pthread_mutex_lock(&mergeNumber);//kleidwnoume gia na prostapsoume thn global metavlhth test
	 if(test == 1){//sto if tha mpei mono an ginetai merge
			pthread_cond_wait(&mergeWait,&mergeNumber);//kai tha perimenei mexri na teleiwsei to merge
	 }
	 pthread_mutex_unlock(&mergeNumber);
	 
	 
    if (memtable_get(self->memtable->list, key, value) == 1){
    		return 1;
    }
    int sstGet = sst_get(self->sst, key, value);
    return sstGet;
}
